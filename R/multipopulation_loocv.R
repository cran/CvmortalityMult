#' Function to apply Leave-One-Out Cross-Validation (LOOCV) technique for testing the forecasting accuracy
...
#'
#' @return A list with class \code{"MultiCv"} including different components of the cross-validation process:
...
#'
#' @seealso \code{\link{multipopulation_cv}},
...
multipopulation_loocv <- function(qxt, model = c("additive", "multiplicative"),
  ...
  if(is.null(qxt) || is.null(model) || is.null(periods) || is.null(ages) ||
     is.null(nPop) || is.null(model) || is.null(ktmethod) || is.null(measures) ||
     is.null(trainset1)){
  ...
  if(trainset1 < 3){
    stop("trainset1 must be a numeric variable and higher than 2.")
  }
  ...
  nrep <- (nperiods - fitting.years1)/1
  ...
  if(prue1 == TRUE){
    ...
    nrep <- floor(nrep) + 1
    test1 <- c(rep(1, nrep - 1))
    test2 <- length(periods) - fitting.years1 - (nrep-1)*1
    test1 <- c(test1, test2)
  }
  ...
  for(reps in 1:nrep){
    ...
    if(model == "additive"){
      ...
      forecast.obj <- forecast.fitLCmulti(object,
                                          nahead = test1[reps],
                                          ktmethod = ktmethod,
  ...
  if(measures == "SSE"){
    ...
    for(i in 1:nPop){
      for(j in 1:(length(periods)-trainset1)){
        meas_prevpops[j,i] <- MeasureAccuracy(measure = "SSE",
                                              qxt_re = mat_qxt[[i]][,(trainset1+j)],
                                              qxt_aju= qxt.forecast[[i]][,j],
                                              wxt = genWeightMat(ages = ages, years = 1, clip = 0))$value

        prev <- (mat_qxt[[i]][,(trainset1+j)] - qxt.forecast[[i]][,j])^2
    ...
    for(j in 1:nages){
      for(pe in 1:(length(periods)-trainset1)){
        for(i in 1:nPop){
          prev <- (mat_qxt[[i]][j,(trainset1+pe)] - qxt.forecast[[i]][j,pe])^2
          prev <- replace(prev, prev == "Inf", 0)
          prev <- replace(prev, prev == "-Inf", 0)
          prev <- replace(prev, prev == "NA", 0)
          prev <- replace(prev, prev == "NaN", 0)
    ...
    for(j in 1:(length(periods)-trainset1)){
      meas_prevtot[j,] <- sum(meas_prevtotal[j,])/(length(qxt.forecast[[i]][,j])*nPop)
    }
    meas_total <- mean(meas_prevtot)
    ...
    for(j in 1:(nperiods-(trainset1))){
      ...
      meas_periods[,j] <- sum(meas_prevperiods[,j])/(nages*nPop)
    }
    ...
    for(i in 1:nPop){
      for(j in 1:(length(periods)-trainset1)){
        meas_prevpops[j,i] <- MeasureAccuracy(measure = "MAE",
                                              ...
                                              qxt_aju= qxt.forecast[[i]][,j],
                                              wxt = genWeightMat(ages = ages, years = 1, clip = 0))$value
        prev <- abs(mat_qxt[[i]][,(trainset1+j)] - qxt.forecast[[i]][,j])
        prev <- replace(prev, prev == "Inf", 0)
        prev <- replace(prev, prev == "-Inf", 0)
        prev <- replace(prev, prev == "NA", 0)
        prev <- replace(prev, prev == "NaN", 0)
    ...
    for(j in 1:(length(periods)-trainset1)){
      meas_prevtot[j,] <- sum(meas_prevtotal[j,])/(nPop*length(mat_qxt[[i]][,(trainset1+j)]))
    }
    meas_total <- mean(meas_prevtot)
    ...
    for(j in 1:nages){
      for(pe in 1:(length(periods)-trainset1)){
        for(i in 1:nPop){
          prev <- abs(mat_qxt[[i]][j,(trainset1+pe)] - qxt.forecast[[i]][j,pe])
          prev <- replace(prev, prev == "Inf", 0)
          prev <- replace(prev, prev == "-Inf", 0)
          prev <- replace(prev, prev == "NA", 0)
          prev <- replace(prev, prev == "NaN", 0)
    ...
    for(i in 1:nPop){
      for(j in 1:(length(periods)-trainset1)){
        meas_prevpops[j,i] <- MeasureAccuracy(measure = "MAPE",
                                              ...
                                              qxt_aju= qxt.forecast[[i]][,j],
                                              wxt = genWeightMat(ages = ages, years = 1, clip = 0))$value
        prev <- abs((mat_qxt[[i]][,(trainset1+j)] - qxt.forecast[[i]][,j])/mat_qxt[[i]][,(trainset1+j)])
        prev <- replace(prev, prev == "Inf", 0)
        prev <- replace(prev, prev == "-Inf", 0)
        prev <- replace(prev, prev == "NA", 0)
        prev <- replace(prev, prev == "NaN", 0)
    ...
    for(j in 1:nages){
      for(pe in 1:(length(periods)-trainset1)){
        for(i in 1:nPop){
          prev <- (abs((mat_qxt[[i]][j,(trainset1+pe)] - qxt.forecast[[i]][j,pe])/mat_qxt[[i]][j,(trainset1+pe)]))
          prev <- replace(prev, prev == "Inf", 0)
          prev <- replace(prev, prev == "-Inf", 0)
          prev <- replace(prev, prev == "NA", 0)
          prev <- replace(prev, prev == "NaN", 0)
    ...
    for(j in 1:(nperiods-(trainset1))){
      for(i in 1:nPop){
        prev <- abs((mat_qxt[[i]][,(trainset1+j)] - qxt.forecast[[i]][,j])/mat_qxt[[i]][,(trainset1+1):length(periods)])
        prev <- replace(prev, prev == "Inf", 0)
        prev <- replace(prev, prev == "-Inf", 0)
        prev <- replace(prev, prev == "NA", 0)
        prev <- replace(prev, prev == "NaN", 0)
    ...
    meas_prevpops1 <- matrix(NA, nrow = (length(periods)-trainset1), ncol=nPop, dimnames = list(c((periods[1] + trainset1):max(df_qxtdata$period)), c(1:nPop)))
    meas_prevpops2 <- matrix(NA, nrow = (length(periods)-trainset1), ncol=nPop, dimnames = list(c((periods[1] + trainset1):max(df_qxtdata$period)), c(1:nPop)))
    meas_prevpops3 <- matrix(NA, nrow = (length(periods)-trainset1), ncol=nPop, dimnames = list(c((periods[1] + trainset1):max(df_qxtdata$period)), c(1:nPop)))
    meas_prevpops4 <- matrix(NA, nrow = (length(periods)-trainset1), ncol=nPop, dimnames = list(c((periods[1] + trainset1):max(df_qxtdata$period)), c(1:nPop)))
    meas_pops <- matrix(NA, nrow = 4, ncol=nPop, dimnames = list(c("SSE", "MSE", "MAE", "MAPE"), c(1:nPop)))

    meas_prevtotal1 <- matrix(NA, nrow = (length(periods)-trainset1), ncol=nPop, dimnames = list(c((periods[1] + trainset1):max(df_qxtdata$period)), c(1:nPop)))
    meas_prevtotal2 <- matrix(NA, nrow = (length(periods)-trainset1), ncol=nPop, dimnames = list(c((periods[1] + trainset1):max(df_qxtdata$period)), c(1:nPop)))
    meas_prevtotal3 <- matrix(NA, nrow = (length(periods)-trainset1), ncol=nPop, dimnames = list(c((periods[1] + trainset1):max(df_qxtdata$period)), c(1:nPop)))
    meas_prevtotal4 <- matrix(NA, nrow = (length(periods)-trainset1), ncol=nPop, dimnames = list(c((periods[1] + trainset1):max(df_qxtdata$period)), c(1:nPop)))

    meas_prevtot <- matrix(NA, nrow = (length(periods)-trainset1), ncol = 4, dimnames = list(c((periods[1] + trainset1):max(df_qxtdata$period)), c("SSE", "MSE", "MAE", "MAPE")))
    meas_total <- matrix(NA, nrow = 4, ncol = 1, dimnames = list(c("SSE", "MSE", "MAE", "MAPE"), "all ages and periods"))

    meas_prevages1 <- matrix(NA, nrow = (length(periods)-trainset1), ncol = nages, dimnames = list(c((periods[1] + trainset1):max(df_qxtdata$period)), ages))
    meas_prevages2 <- matrix(NA, nrow = (length(periods)-trainset1), ncol = nages, dimnames = list(c((periods[1] + trainset1):max(df_qxtdata$period)), ages))
    meas_prevages3 <- matrix(NA, nrow = (length(periods)-trainset1), ncol = nages, dimnames = list(c((periods[1] + trainset1):max(df_qxtdata$period)), ages))
    meas_prevages4 <- matrix(NA, nrow = (length(periods)-trainset1), ncol = nages, dimnames = list(c((periods[1] + trainset1):max(df_qxtdata$period)), ages))
    ...
    meas_prevperiods1 <- matrix(NA, nrow = nPop, ncol = (nperiods-(trainset1)), dimnames = list(c(1:nPop), c((periods[1] + trainset1):max(df_qxtdata$period))))
    meas_prevperiods2 <- matrix(NA, nrow = nPop, ncol = (nperiods-(trainset1)), dimnames = list(c(1:nPop), c((periods[1] + trainset1):max(df_qxtdata$period))))
    meas_prevperiods3 <- matrix(NA, nrow = nPop, ncol = (nperiods-(trainset1)), dimnames = list(c(1:nPop), c((periods[1] + trainset1):max(df_qxtdata$period))))
    meas_prevperiods4 <- matrix(NA, nrow = nPop, ncol = (nperiods-(trainset1)), dimnames = list(c(1:nPop), c((periods[1] + trainset1):max(df_qxtdata$period))))

    meas_periods <- matrix(NA, nrow = 4, ncol = (nperiods-(trainset1)), dimnames = list(c("SSE", "MSE", "MAE", "MAPE"), c((periods[1] + trainset1):max(df_qxtdata$period))))

    wxt <- genWeightMat(ages = ages, years = c((periods[1] + trainset1):max(df_qxtdata$period)), clip = 0)
    for(i in 1:nPop){
      for(j in 1:(length(periods)-trainset1)){
        meas_prevpops1[j,i] <- MeasureAccuracy(measure = "SSE",
                                               qxt_re = mat_qxt[[i]][,(trainset1+j)],
                                               qxt_aju = qxt.forecast[[i]][,j],
                                               wxt = genWeightMat(ages = ages, years = 1, clip = 0))$value
        meas_prevpops2[j,i] <- MeasureAccuracy(measure = "MSE",
                                               qxt_re = mat_qxt[[i]][,(trainset1+j)],
                                               qxt_aju = qxt.forecast[[i]][,j],
                                               wxt = genWeightMat(ages = ages, years = 1, clip = 0))$value
        meas_prevpops3[j,i] <- MeasureAccuracy(measure = "MAE", qxt_re = mat_qxt[[i]][,(trainset1+j)],
                                               qxt_aju = qxt.forecast[[i]][,j],
                                               wxt = genWeightMat(ages = ages, years = 1, clip = 0))$value
        meas_prevpops4[j,i] <- MeasureAccuracy(measure = "MAPE", qxt_re = mat_qxt[[i]][,(trainset1+j)],
                                               qxt_aju = qxt.forecast[[i]][,j],
                                               wxt = genWeightMat(ages = ages, years = 1, clip = 0))$value
        #SSE
        prev1 <- (mat_qxt[[i]][,(trainset1+j)] - qxt.forecast[[i]][,j])^2
        ...
        #MSE
        prev2 <- (mat_qxt[[i]][,(trainset1+j)] - qxt.forecast[[i]][,j])^2
        ...
        #MAE
        prev3 <- abs(mat_qxt[[i]][,(trainset1+j)] - qxt.forecast[[i]][,j])
        ...
        #MAPE
        prev4 <- abs((mat_qxt[[i]][,(trainset1+j)] - qxt.forecast[[i]][,j])/mat_qxt[[i]][,(trainset1+j)])
    ...
    for(j in 1:(length(periods)-trainset1)){
      ...
      meas_prevtot[j,2] <- sum(meas_prevtotal2[j,])/(nPop*length(mat_qxt[[i]][,(trainset1+j)]))
      meas_prevtot[j,3] <- sum(meas_prevtotal3[j,])/(nPop*length(mat_qxt[[i]][,(trainset1+j)]))
      meas_prevtot[j,4] <- sum(meas_prevtotal4[j,])/(nPop*length(mat_qxt[[i]][,(trainset1+j)]))
    ...
    for(j in 1:nages){
      for(pe in 1:(length(periods)-trainset1)){
        for(i in 1:nPop){
          #SSE
          prev1 <- (mat_qxt[[i]][j,(trainset1+pe)] - qxt.forecast[[i]][j,pe])^2
          prev1 <- replace(prev1, prev1 == "Inf", 0)
          prev1 <- replace(prev1, prev1 == "-Inf", 0)
          prev1 <- replace(prev1, prev1 == "NA", 0)
          prev1 <- replace(prev1, prev1 == "NaN", 0)
          ...
          #MSE
          prev2 <- (mat_qxt[[i]][j,(trainset1+pe)] - qxt.forecast[[i]][j,pe])^2
          prev2 <- replace(prev2, prev2 == "Inf", 0)
          prev2 <- replace(prev2, prev2 == "-Inf", 0)
          prev2 <- replace(prev2, prev2 == "NA", 0)
          prev2 <- replace(prev2, prev2 == "NaN", 0)
          ...
          #MAE
          prev3 <- abs(mat_qxt[[i]][j,(trainset1+pe)] - qxt.forecast[[i]][j,pe])
          prev3 <- replace(prev3, prev3 == "Inf", 0)
          prev3 <- replace(prev3, prev3 == "-Inf", 0)
          prev3 <- replace(prev3, prev3 == "NA", 0)
          prev3 <- replace(prev3, prev3 == "NaN", 0)
          ...
          #MAPE
          prev4 <- abs((mat_qxt[[i]][j,(trainset1+pe)] - qxt.forecast[[i]][j,pe])/mat_qxt[[i]][j,(trainset1+pe)])
          prev4 <- replace(prev4, prev4 == "Inf", 0)
          prev4 <- replace(prev4, prev4 == "-Inf", 0)
          prev4 <- replace(prev4, prev4 == "NA", 0)
          prev4 <- replace(prev4, prev4 == "NaN", 0)
        ...
        meas_prevages2[pe,j] <- sum(meas_prevag[,2])/length(meas_prevag)
        meas_prevages3[pe,j] <- sum(meas_prevag[,3])/length(meas_prevag)
        meas_prevages4[pe,j] <- sum(meas_prevag[,4])/length(meas_prevag)
      }
    ...
    for(j in 1:(nperiods-(trainset1))){
      for(i in 1:nPop){
        ...
        #MAPE
        prev4 <- abs((mat_qxt[[i]][,(trainset1+j)] - qxt.forecast[[i]][,j])/mat_qxt[[i]][,(trainset1+1):length(periods)])
        prev4 <- replace(prev4, prev4 == "Inf", 0)
        prev4 <- replace(prev4, prev4 == "-Inf", 0)
        prev4 <- replace(prev4, prev4 == "NA", 0)
        prev4 <- replace(prev4, prev4 == "NaN", 0)
      ...
      meas_periods[2,j] <- sum(meas_prevperiods2[,j])/(nages*nPop)
